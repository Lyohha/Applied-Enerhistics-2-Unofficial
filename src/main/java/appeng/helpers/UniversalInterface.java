...
public class UniversalInterface implements IGridTickable, IStorageMonitorable, IInventoryDestination, IAEAppEngFluidHandler, IConfigManagerHost, ICraftingProvider, IUpgradeableHost, IPriorityHost
{
	...
	private final IUniversalInterfaceHost iHost;
	private final BaseActionSource mySource;
	private final BaseActionSource interfaceRequestSource;
	private final ConfigManager cm = new ConfigManager( this );
	private final AppEngInternalAEInventory config = new AppEngUniversalInternalAEInventory( this, NUMBER_OF_CONFIG_SLOTS, this.cm );
	private final AppEngInternalUniversalInventory storage = new AppEngInternalUniversalInventory(this, NUMBER_OF_STORAGE_SLOTS);
	...
	public UniversalInterface(final AENetworkProxy networkProxy, final IUniversalInterfaceHost ih )
	{
		...
		this.cm.registerSetting( Settings.INTERFACE_FLUID_MODE, YesNo.YES );
		...
		this.interfaceRequestSource = new UniversalInterface.InterfaceRequestSource( this.iHost );
	}
	...
	private void updatePlan( final int slot )
	{
		...
		else if( req != null )
		{
			...
			{
				if(req.getItem() instanceof ItemFluid)
				{
					FluidTankInfo info = this.storage.getTankInfo(slot);
					if(info.fluid == null || info.capacity > info.fluid.amount) {

						ItemStack itemStack = req.copy().getItemStack();

						NBTTagCompound tag = new NBTTagCompound();
						tag.setInteger("mFluidDisplayAmount", info.capacity - (info.fluid == null ? 0: info.fluid.amount));
						itemStack.setTagCompound(tag);

						this.requireWork[slot] = AEApi.instance().storage().createItemStack(itemStack);
			...
			else if( req.isSameType( Stored ) || req.getItem() instanceof ItemFluid) // same type ( qty different? )!
			{
				if(req.getItem() instanceof ItemFluid)
				{
					FluidTankInfo info = this.storage.getTankInfo(slot);
				...
				else if( req.getStackSize() != Stored.stackSize )
				{
					this.requireWork[slot] = req.copy();
	...
	private boolean usePlan( final int x, final IAEItemStack itemStack )
	{
		...
		try
		{
			...
			{
				if(itemStack.getItemStack().getItem() instanceof ItemFluid)
				{
					NBTTagCompound data = itemStack.getItemStack().getTagCompound();
					if(data != null) {
						if (!data.hasKey("mFluidDisplayAmount"))
							throw new IllegalStateException("Not set fluid amount in tag");
						FluidStack stack = new FluidStack(FluidRegistry.getFluid(itemStack.getItemStack().getItemDamage()), (int) data.getLong("mFluidDisplayAmount"));

						IAEFluidStack iFluidStack = AEApi.instance().storage().createFluidStack(stack);

						IAEFluidStack drain = this.gridProxy.getStorage().getFluidInventory().extractItems(iFluidStack, Actionable.SIMULATE, new MachineSource(this.iHost));

						if (drain != null && drain.getFluidStack().amount > 0) {

							drain = this.gridProxy.getStorage().getFluidInventory().extractItems(drain, Actionable.MODULATE, new MachineSource(this.iHost));

							this.storage.fill(x, drain.getFluidStack(), true);
			...
			else if( itemStack.getStackSize() < 0 )
			{
				...
				if(itemStack.getItemStack().getItem() instanceof ItemFluid)
				{
					// достать информацию про жидкость
					FluidStack stack = this.storage.getTankInfo(ForgeDirection.UNKNOWN)[x].fluid;
					if(stack != null) {
						// сделать стак
						IAEFluidStack iFluidStack = AEApi.instance().storage().createFluidStack(stack);
						// смоделировать наполнение
						IAEFluidStack notAdded = this.gridProxy.getStorage().getFluidInventory().injectItems(iFluidStack, Actionable.SIMULATE, new MachineSource(this.iHost));
						int drain = stack.amount;

						// изменить объемы если все не влезло
						if(notAdded != null)
							drain -= notAdded.getStackSize();

						// забрать жидкость с танка
						iFluidStack = AEApi.instance().storage().createFluidStack(this.storage.drain(x, drain, true));

						// отправить жидкость в систему
						this.gridProxy.getStorage().getFluidInventory().injectItems(iFluidStack, Actionable.MODULATE, new MachineSource(this.iHost));
						changed = true;
					}
	...
	public IMEMonitor<IAEItemStack> getItemInventory()
	{
		if( this.hasConfig() )
		{
			return new UniversalInterface.InterfaceInventory( this );
		}
	...
	public IAEUniversalFluidHandler getFluidStorage() {
		return this.storage;
	}
	...
	public IStorageMonitorable getMonitorable( final ForgeDirection side, final BaseActionSource src, final IStorageMonitorable myInterface )
	{
		...
		final UniversalInterface di = this;
		...
		return new IStorageMonitorable()
		{
			...
			public IMEMonitor<IAEItemStack> getItemInventory()
			{
				return new UniversalInterface.InterfaceInventory( di );
			}
	...
	public boolean pushPattern( final ICraftingPatternDetails patternDetails, final InventoryCrafting table )
	{
		...
		for( final ForgeDirection s : possibleDirections )
		{
			...
			if( te instanceof IUniversalInterfaceHost )
			{
				try
				{
					if( ( (IUniversalInterfaceHost) te ).getInterfaceDuality().sameGrid( this.gridProxy.getGrid() ) )
					{
						continue;
					}
				}
	...
	public void addDrops( final List<ItemStack> drops )
	{
		...
		{
			if( is != null && !(is.getItem() instanceof ItemFluid))
			{
				drops.add( is );
			}
		}
	...
	public String getTermName()
	{
		...
		for( final ForgeDirection direction : possibleDirections )
		{
			...
			if( directedTile instanceof IUniversalInterfaceHost )
			{
				try
				{
					if( ( (IUniversalInterfaceHost) directedTile ).getInterfaceDuality().sameGrid( this.gridProxy.getGrid() ) )
					{
						continue;
					}
				}
	...
	private class InterfaceInventory extends MEMonitorIInventory
	{
		...
		public InterfaceInventory( final UniversalInterface tileInterface )
		{
			super( new AdaptorIInventory( tileInterface.storage ) );
			this.setActionSource( new MachineSource( UniversalInterface.this.iHost ) );
		}

		@Override
		public IAEItemStack injectItems( final IAEItemStack input, final Actionable type, final BaseActionSource src )
		{
			if( src instanceof UniversalInterface.InterfaceRequestSource)
			{
				return input;
			}
